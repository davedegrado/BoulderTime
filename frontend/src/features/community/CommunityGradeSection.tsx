...
export function CommunityGradeSection({ boulderId }: { boulderId: string }) {
  ...
  if (consensus.isPending) return <LoadingState label="Loading community grade" />;
  ...
  return (
    <section className="section card" aria-labelledby="community-grade-title">
      <h2 id="community-grade-title" className="section__title">Community grade</h2>
      <p className="field__hint">What climbers think. The official grade is set by the gym and doesn't change.</p>
      {data.systems.map((s) => <SystemBlock key={s.gradeSystemId} boulderId={boulderId} system={s} canSuggest={data.viewerCanSuggest} />)}
      {session && !data.viewerCanSuggest && <p className="field__hint">Log an attempt to suggest a grade.</p>}
    </section>
  );
}
  ...
  return (
    <div className="consensus">
      <div className="consensus__head">
        <p className="list__title">{s.systemName}</p>
        <p className="list__sub">{s.totalVotes === 0 ? "No votes yet" : `${s.totalVotes} ${s.totalVotes === 1 ? "vote" : "votes"}${consensus ? ` · consensus ${consensus.label}` : ""}`}</p>
      </div>
      {s.buckets.length > 0 && (
        <ul className="consensus__bars" aria-label={`${s.systemName} votes`}>
          {s.buckets.map((b) => (
            <li key={b.gradeValueId} className={`consensus__row ${b.gradeValueId === s.consensusValueId ? "is-consensus" : ""}`}>
              <span className="consensus__label">{b.label}</span>
              ...
              {b.gradeValueId === s.officialValueId && <span className="tag tag--dark">Official</span>}
            </li>
          ))}
        </ul>
      )}
      {canSuggest && (
        <SelectField label={`Your ${s.systemName} grade`} value={s.viewerValueId ?? ""} disabled={suggest.isPending}
