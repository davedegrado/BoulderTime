...
export function CommentsSection({ boulderId }: { boulderId: string }) {
  ...
  return (
    <section className="section" aria-labelledby="comments-title">
      ...
      {comments.hasNextPage && <Button variant="secondary" onClick={() => comments.fetchNextPage()} loading={comments.isFetchingNextPage}>{t("Older comments")}</Button>}

      {session ? (
        <form className="composer" onSubmit={onSubmit} noValidate>
          <label className="sr-only" htmlFor="comment-draft">{t("Write a comment")}</label>
          <textarea id="comment-draft" className="field__input field__textarea composer__input" rows={2} maxLength={1000}
            placeholder={t("Add a comment…")} value={draft} onChange={(e) => setDraft(e.target.value)} />
          <Button type="submit" icon={<Send aria-hidden />} loading={m.create.isPending} disabled={!draft.trim()} aria-label={t("Post comment")}>{t("Post")}</Button>
        </form>
      ) : (
        <Link to={`/sign-in?next=${encodeURIComponent(location.pathname)}`} className="btn btn--secondary"><span>{t("Sign in to comment")}</span></Link>
      )}
    </section>
  );
}
      ...
      <div className="comment__body">
        ...
        {editing ? (
          <form className="composer" onSubmit={(e) => { e.preventDefault(); m.edit.mutate({ id: c.id, content: text }, { onSuccess: () => setEditing(false), onError }); }}>
            <textarea className="field__input field__textarea composer__input" rows={2} maxLength={1000} value={text} onChange={(e) => setText(e.target.value)} aria-label={t("Edit comment")} />
            <Button type="submit" loading={m.edit.isPending} disabled={!text.trim()}>{t("Save")}</Button>
            <Button variant="ghost" onClick={() => { setEditing(false); setText(c.content); }}>{t("Cancel")}</Button>
          </form>
        ...
        {!editing && (
          <div className="comment__actions">
            ...
            {c.isMine && <button type="button" className="text-btn" onClick={() => setEditing(true)}><Pencil aria-hidden /> {t("Edit")}</button>}
            {c.isMine && <ConfirmButton icon={<Trash2 aria-hidden />} confirmLabel={t("Delete?")} loading={m.remove.isPending} onConfirm={() => m.remove.mutate(c.id, { onError })}>{t("Delete")}</ConfirmButton>}
            {c.canModerate && (
              <button type="button" className="text-btn" onClick={() => m.hide.mutate({ id: c.id, hide: c.status !== "HIDDEN" }, { onError })}>
                {c.status === "HIDDEN" ? <><Eye aria-hidden /> {t("Unhide")}</> : <><EyeOff aria-hidden /> {t("Hide")}</>}
              </button>
            )}
