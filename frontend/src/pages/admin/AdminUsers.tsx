...
export function AdminUsers() {
  ...
  return (
    <div className="stack">
      <SearchField label="Search users" placeholder="Name or email" value={text} onChange={setText} />
      {users.isPending ? <LoadingState label="Loading users" />
        : users.isError ? <ErrorState error={users.error} onRetry={() => users.refetch()} />
        : users.data.items.length === 0 ? <EmptyState icon={<UserRound />} title="No users found" />
        : (
          <>
            ...
            <ul className="list">
              {users.data.items.map((u) => (
                <li key={u.id} className="list__row">
                  ...
                  {u.isPlatformAdmin && <Badge tone="dark"><ShieldCheck aria-hidden /> Admin</Badge>}
                </li>
              ))}
            </ul>
          </>
        )}
      <p className="section__footnote">Platform admins are granted from the server command line, never from this screen.</p>
    </div>
  );
}
