...
export function ActivityPage() {
  ...
  return <ClimbingActivity userId={me.data.id} title="Your climbing" />;
}
...
export function ClimbingActivity({ userId, title, header }: { userId: string; title?: string; header?: ReactNode }) {
  ...
  if (profile.isPending) return <LoadingState label="Loading activity" />;
  ...
  return (
    <div className="page">
      ...
      <div className="stats">
        <div className="stat"><span className="stat__value">{p.stats.completed}</span><span className="stat__label">Completed</span></div>
        <div className="stat"><span className="stat__value">{p.stats.projects}</span><span className="stat__label">Projects</span></div>
        <div className="stat"><span className="stat__value">{p.stats.completedThisMonth}</span><span className="stat__label">Sent this month</span></div>
        <div className="stat"><span className="stat__value">{p.stats.totalAttempts}</span><span className="stat__label">Attempts</span></div>
      </div>

      <section className="section card" aria-labelledby="weekly-title">
        <h2 id="weekly-title" className="section__title">Last 12 weeks</h2>
        <WeeklyChart weeks={p.weekly} />
      </section>

      <section className="section card" aria-labelledby="highest-title">
        <h2 id="highest-title" className="section__title">Highest grades</h2>
        <HighestGrades grades={p.highestGrades} />
      </section>

      <section className="section" aria-labelledby="history-title">
        <h2 id="history-title" className="section__title">History</h2>
        <div className="chips" role="radiogroup" aria-label="Filter history">
        ...
        {history.isPending ? <LoadingState label="Loading history" />
          ...
          : items.length === 0 ? (
            <EmptyState icon={<Mountain />} title={filter === "PROJECTS" ? "No open projects" : "No climbs logged yet"}
              ...
              action={p.isMe ? <Link to="/explore" className="btn btn--primary"><span>Find a gym</span></Link> : undefined} />
          ) : (
            <>
              <ul className="history">{items.map((i) => <HistoryRow key={i.boulder.id} item={i} />)}</ul>
              {history.hasNextPage && <Button variant="secondary" onClick={() => history.fetchNextPage()} loading={history.isFetchingNextPage}>Show more</Button>}
            </>
          )}
      </section>
    </div>
